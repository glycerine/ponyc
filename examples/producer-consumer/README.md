...
Q: How do I write an assertion, given that pony programs are guaranteed
at compile time to not abort early?

A: the easiest thing I have found it to call out to C to
enforce an assertion immediately. This avoids having to radically
change the program to use the pony_test package:

primitive Assert
  fun equal[T: (Equatable[T] #read & Stringable #read)](got:T, want:T) =>
    if got != want then
      // try to get our own line for better visibility.
      @fflush[I32](@pony_os_stdout())
      @fflush[I32](@pony_os_stderr())
      let msg = "error: Assert.equal violated! want: " + want.string() + ", but got: " + got.string() + "\n"
      @write(I32(2), msg.cstring(), msg.size())  // 2 = stderr
      @exit(1)
    end 

where the top of the file declares
...
An assertion can then be written over an I64 as follows:

Assert.equal[I64](got, want)

To use "pony_test" instead, see
https://tutorial.ponylang.io/appendices/examples#how-to-write-tests

Note that even when using the C FFI, your assert message may
